...
class BleManager extends NativeEventEmitter {
	...
	createBond(peripheralId: string, peripheralPin: string | null = null) {
		return new Promise<void>((fulfill, reject) => {
			bleManager
				.createBond(peripheralId, peripheralPin)
				.then(() => {
					fulfill();
				})
				.catch((error) => {
	 ...
	 * @returns
	 */
	startNotificationUseBuffer(peripheralId: string, serviceUUID: string, characteristicUUID: string, buffer: number) {
		return new Promise<void>((fulfill, reject) => {
			bleManager
	 ...
	 * @returns
	 */
	scan(serviceUUIDs: string[], seconds: number, allowDuplicates?: boolean, scanningOptions: ScanOptions = {}) {
		return new Promise<void>((fulfill, reject) => {
	...
	requestConnectionPriority(peripheralId: string, connectionPriority: ConnectionPriority) {
		return new Promise<boolean>((fulfill, reject) => {
			bleManager.requestConnectionPriority(peripheralId, connectionPriority).then(() => {
				console.log('harmony is not support');
			});
		});
	}
	...
	getMaximumWriteValueLengthForWithoutResponse(peripheralId: string) {
		return new Promise<number>((fulfill, reject) => {
			bleManager.getMaximumWriteValueLengthForWithoutResponse(peripheralId).then(() => {
				console.log('harmony is not support');
			});
		});
	}
	...
	getMaximumWriteValueLengthForWithResponse(peripheralId: string) {
		return new Promise<number>((fulfill, reject) => {
			bleManager.getMaximumWriteValueLengthForWithResponse(peripheralId).then(() => {
				console.log('harmony is not support');
			});
		});
	}
